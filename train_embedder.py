...
class PoemImageEmbedTrainer():
    def __init__(self, train_data, test_data, load_model, device):
        self.device = device
        ...
        self.train_set = PoemImageEmbedDataset(self.train_data, img_dir,
                                               tokenizer=self.tokenizer, max_seq_len=200,
                                               transform=self.train_transform)
        self.train_loader = DataLoader(self.train_set, batch_size=4, shuffle=True, num_workers=4)

        self.test_set = PoemImageEmbedDataset(self.test_data, img_dir,
                                              tokenizer=self.tokenizer, max_seq_len=200,
                                              transform=self.test_transform)
        self.test_loader = DataLoader(self.test_set, batch_size=4, num_workers=4)

        self.model = PoemImageEmbedModel()
        self.model = DataParallel(self.model)
        if load_model:
            self.model.load_state_dict(torch.load(load_model))
        self.model.to(device)
        self.optimizer = optim.Adam(list(self.model.module.poem_embedder.linear.parameters()) + \
                                    list(self.model.module.img_embedder.linear.parameters()))
    ...
    def train_epoch(self, epoch, log_interval, save_interval, ckpt_file):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            loss = self.model.module.calc_loss(img1, ids1, mask1, img2, ids2, mask2)
            ...
            if (i + 1) % log_interval == 0:
                ...
                print('[{:>2}, {:>4}/{}] loss:{:.4}  {:.3}iters/s {}left'.format(
                    epoch, (i + 1), num_batches, running_ls / log_interval,
...
def main():
    ...
    embed_trainer = PoemImageEmbedTrainer(train_data, test_data, args.load_model, device)
